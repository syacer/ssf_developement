...
static void find_as(u8* argv0) {
  ...
  if (!access(SSF_INST_PATH "/as", X_OK)) {
    as_path = SSF_INST_PATH;
    return;
  }
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "  CC=%s/compile_wrap ./configure\n"
         "  CXX=%s/compile_wrap ./configure\n",
         SSF_INST_PATH, SSF_INST_PATH);
