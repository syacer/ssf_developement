...
if (!stop_soon) {
  ret = sendnbytes(&fakechar, 1);
  if (ret < 0) ABORT("fifo error: %s \n",strerror(errno));
}
...
if (!stop_soon) {
  ...
  struct stat *st = NULL;
  ret = recvnbytes(buf, MAX_BUF_SIZE);
  if (strcmp(buf, REST)) {
    
    if (stat(buf, st)) {
      add_to_queue(buf, st->st_size, 0);
    } else {
      SAYF("can't read test file %s, errorcode: %d", buf, errno);
    }
  }
...
int tmpArgc = argc - optind;
passargvtoclient(argv + optind, &tmpArgc);
